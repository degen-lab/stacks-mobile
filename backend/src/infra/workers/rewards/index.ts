...
AppDataSource.initialize()
  .then(async (dataSource) => {
    ...
    const jobPattern = '0 0 1,15 * *'; // 1st and 15th of each month at 00:00
    repeatableJobs.some(
      (job) =>
    ...
    await rewardsQueue.add(
      ...
      {},
      {
        repeat: { pattern: jobPattern }, // 1st and 15th of each month at 00:00
        attempts: 3,
        backoff: {
          type: 'exponential',
          delay: 2000,
    ...
    logger.info(
      'Repeatable job processTournamentCycle scheduled (at 00:00 on days 1 and 15)',
    );
