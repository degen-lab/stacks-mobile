...
import { DefiOperationNotFoundError, SwapRouteNotFoundError } from "../errors/defiErrors";
import { EntityManager } from "typeorm";
import { TransactionClientPort } from "../ports/transactionClientPort";
import { DefiOperationType, TransactionStatus } from "../../domain/entities/enums";
...
export class DefiService {
  ...
  async getSwapParams(userId: number, tokenInId: string, tokenOutId: string, senderAddress: string, amount: number): Promise<{
  ...
  async saveDefiOperation(defiOperationId: number, txId: string): Promise<void> {
    ...
    if (!defiOperation) {
      throw new DefiOperationNotFoundError(`Defi operation with id ${defiOperationId} not found`);
    }

    const transactionStatus = await this.transactionClient.getTransactionStatus(txId);
