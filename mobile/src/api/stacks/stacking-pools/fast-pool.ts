import { principalCV, PostConditionMode, uintCV } from "@stacks/transactions";
...
export class FastPoolService implements IPoolService {
  ...
  async isCallerAllowed(userAddress: string): Promise<boolean> {
    ...
    try {
      return await fetchReadOnly<boolean>(
        address,
        name,
        SC_FUNCTIONS.stackingFastPool.readOnlyFunctions.CHECK_CALLER_ALLOWED,
        [principalCV(userAddress)],
      );
    } catch (error) {
